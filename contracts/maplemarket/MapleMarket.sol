...
import "./MapleBase.sol";

contract MapleMarket is MapleBase {
    ...
    constructor(address _accountFactory, address _JEWEL) MapleBase(_accountFactory) {
        JEWEL = IERC20(_JEWEL);
    }
    ...
    function cancelOffer(uint256 offerId) external {
        ...
        require(offer.status == OfferStatus.Created, "invalid offer status");
        address account = getAccount(msg.sender);
        require(account == offer.creator, "no permission");
    ...
    function bid(uint256 offerId) external {
        ...
        require(jewelBalance >= cost, "insufficient jewel balance");
    ...
    function changeOfferCost(uint256 offerId, uint256 _costInJewel) external {
        require(offerId < numOffers, "invalid offerId");
        Offer memory offer = offers[offerId];
        require(offer.status == OfferStatus.Created, "invalid offer status");
        address account = getAccount(msg.sender);
        require(account == offer.creator, "no permission");
        require(_costInJewel > 0, "zero cost");
